    ...
    async function updateStats() {
      ...
      let topYear = null, topYearCount = 0;
    ...
    async function shortenUrl() {
      ...
      document.getElementById("shareTwitter").onclick = () => {
        window.open(`https://twitter.com/intent/tweet?text=Check%20this%20link:%20${encodeURIComponent(shortLink)}`, '_blank');
      };
      document.getElementById("shareFacebook").onclick = () => {
        window.open(`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(shortLink)}`, '_blank');
      };
      document.getElementById("shareWhatsApp").onclick = () => {
        window.open(`https://api.whatsapp.com/send?text=Check%20this%20link:%20${encodeURIComponent(shortLink)}`, '_blank');
      };
      openModal();
    ...
    // Fungsi checkRedirect untuk mendukung path rewriting
    (async function checkRedirect() {
      ...
      if (path && path !== "/") {
        ...
        if (docSnap.exists()) {
          ...
          document.body.innerHTML = "<h2 style='color: #fff; text-align: center; margin-top: 50px;'>Link tidak ditemukan!</h2>";
        }
      } else {
